 ...
 *
 * This file contains the make first comment script for AJAX calls.
 *
 * Aaron Mitchell 2018
...
require_once("commentboard.php");
			...
			// If this is not a signed-in user, no first comment.
...
$username = $commenter->username;
			...
			// If this is not POST, no first comment.
...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {			
 ...
 die();
			...
			// If this board does not exist, no first comment.
...
if ( !file_exists(RLC_SECRET_PATH."/boards/".$_POST["ancestor"]) ) {
 ...
 die();
			...
			// instantiate the board and make first comment.

$commentBoard = new CommentBoard($_POST["ancestor"]);
if ( $id = $commentBoard->make_comment(
		$_POST["fc"], $username) ) {
