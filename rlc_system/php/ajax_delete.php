 ...
 *
 * This file contains the delete script for AJAX calls.
 *
 * Aaron Mitchell 2018
 ...
 || !($commenter->isMod) ) 
{
 ...
 die();
			...
			// If this is not POST, no delete allowed.
...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
 ...
 die();
			...
			// If this board does not exist, no delete allowed.
			// Echo JSON status error "invalid board" and die.

if ( !file_exists(RLC_SECRET_PATH."/boards/".$_POST["ancestor"]) ) {
 echo '{"status":"invalid board"}';
 die();
}
			// If this comment does not exist, no delete allowed.
			// Echo JSON status error "invalid comment" and die.
	...
	$_POST["ancestor"]."/".$_POST["id"].".txt") ) 
{
 ...
 die();
			...
			// If everything has checked out so far, 			// instantiate and load the comment.
